...
function findIndexByProperty(arr, name) {
  ...
  const foundIndex = arr.findIndex((inxObj) => inxObj.name === name);
...
function findLastEvenNumber(arr) {
  ...
  const findNumber = arr.findLast((evenNum) => evenNum % 2 === 0);

  return findNumber || "Парних чисел не знайдено";
}
...
function getArrayKeys(arr) {
  ...
  const findKeys = arr.keys();
  const arrayKeys = Array.from(findKeys);
...
function getPositiveArrayValues(arr) {
  ...
  const findVal = arr.values();
  const convertArr = Array.from(findVal);
  const positiveArrayValues = convertArr.filter((value) => value > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  const cutElem = arr.splice(startIdx);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const squaredArr = alignArr.map((elem) => elem ** 2);
  const filteredSquaredArray = squaredArr.filter((value) => value > threshold);
...
function findLastNegativeNumberIndex(arr) {
  ...
  const lastIndex = arr.findLastIndex((numIndex) => numIndex < 0);
  return lastIndex;
}
